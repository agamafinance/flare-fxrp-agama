...
contract FdcIntegrationTest is Test {
    ...
    function test_XrpOnRamp_RejectsUnattestedDeposit() public {
        XrpOnRamp ramp = new XrpOnRamp(keccak256("agama-xrpl-address"));
        assertGt(address(ramp.fdc()).code.length, 0, "gateway wired to live FdcVerification");
        // a deposit with no valid FDC proof is rejected (verifyPayment fails)
        Payment.Proof memory empty;
        vm.expectRevert();
        ramp.depositWithXrpProof(empty, address(this));
    }
}
